...
load_data <- function(file_url, data_dir = 'data', cache = FALSE){
  ...
  # as_tibble(
    # transform(
    #   read.csv(file_path),
    #   # data.table(read.csv(file_path)), # fread(xzfile(file_path))
    #   # id = as.character(id),
    ...
    # )
    # data.table(read.csv(file_path))[,
    fread(paste('xz -T 0 -d -c', file_path))[,
        c('zipcode', 'date', 'year', 'month', 'week') := list(
            factor(zipcode),
            as.integer(substring(as.character(date), 1, 8)),
            as.integer(substring(as.character(date), 1, 4)),
            as.integer(substring(as.character(date), 5, 6)),
            as.integer(format(as.Date(substring(as.character(date), 1, 8), format = '%Y%m%d'), '%W'))
        )
    ]
  # )
}
...
# house <- raw_house[, -nearZeroVar(raw_house)] # data.frame
house <- raw_house[, !nearZeroVar(raw_house), with=FALSE] # data.table
setkey(house, price)
...
price.mean <- function(dataset) as.data.table(within(
    cbind(
...
# TODO : to convert ot a data.table
...
validation.price.mean <- merge(
    validation.gbm[, price.gbm := as.integer(round(price.gbm))],
    validation[, price.lm := predLm][, .(price = as.integer(round(mean(price))), price.lm = as.integer(round(mean(price.lm)))), by = zipcode],
    by = 'zipcode'
) # data.table
...
server <- function(input, output, session) {
  ...
  # })
    ...
    price.subset <- function(dataset, price_range){
        if(is.null(price_range)) price_range <- range(validation$price)
        # dataset[dataset$price >= price_range[1] & dataset$price <= price_range[2], ] # data.frame
        dataset[price %between% price_range] # data.table
    }

    dataset.custom <- function(dataset, input, model)
        if(!is.null(input$zipcode)) {
            # TODO : detail what's going on here
            # dataset <- dataset[dataset$zipcode == input$zipcode, c('grade', 'condition', 'zipcode', 'sqft_lot', 'bedrooms', 'week')][1, ] # data.frame
            # dataset[, c('grade', 'condition', 'sqft_lot', 'bedrooms', 'week')] <- c(input$grade, input$condition, input$sqft_lot, input$bedrooms, input$week) # data.frame
            # dataset[, .(grade, condition, sqft_lot, bedrooms, week)] <- c(input$grade, input$condition, input$sqft_lot, input$bedrooms, input$week) # data.frame
            # transform(
            #   # cbind(dataset, price = predict(model, dataset)), # data.frame
            #   dataset[, price := predict(model, dataset)],
            #   zipcode = as.numeric(levels(zipcode))[zipcode]
            # ) # data.frame
            dataset <- dataset[dataset$zipcode == input$zipcode, .(zipcode)][1, ][, c('grade', 'condition', 'sqft_lot', 'bedrooms', 'week') := list(input$grade, input$condition, input$sqft_lot, input$bedrooms, input$week)]
            dataset[, c('price', 'zipcode') := list(predict(model, dataset), as.numeric(levels(zipcode))[zipcode])]
        }
    ...
    validation.react <- reactive({
        # dataset <- transform(validation.price.mean, zipcode = as.numeric(levels(zipcode))[zipcode]) # data.frame
    ...
    output$distPrice <- renderPlot({
        ...
        price <- house.react()$price.gbm # data.frame
        # price <- house.react()[, price.gbm] # data.table
