...
emissions_year_baltimore <- aggregate(Emissions ~ year + type, NEI[NEI$fips == '24510', ], sum, na.rm = T)
png('pics/plot3.png')# , width = 480, height = 480, units = 'px'
# # Plot the chart
# plot(emissions_year_baltimore, type = 'o', main = 'Total PM2.5 emission in Baltimore City', xlab = 'Year of emissions recorded', ylab = 'Amount of PM2.5 emitted, in tons', col = NEI$type)
# with(emissions_year_baltimore, plot(year, Emissions, type = 'n', main = 'Total PM2.5 emission in Baltimore City', xlab = 'Year of emissions recorded', ylab = 'Amount of PM2.5 emitted, in tons'))
# for(t in levels(emissions_year_baltimore$type))
#   with(subset(emissions_year_baltimore, type == t), lines(year, Emissions, type = 'b', col = match(t, levels(NEI$type))))
# legend('topright', lwd = 1, col = seq_along(levels(emissions_year_baltimore$type)), legend = levels(emissions_year_baltimore$type))

print(
  ggplot(data=emissions_year_baltimore, aes(x=year, y=Emissions, group=type, colour=type)) +
    geom_line() + geom_point() +
    labs(title = 'Total PM2.5 emission in Baltimore City', x = 'Year of emissions recorded', y = 'Amount of PM2.5 emitted, in tons')
)
dev.off()
