...
# subset the emisions data on coal emission types
coal <- merge(NEI, SCC[grep('Coal', SCC[, 'Short.Name']), c('SCC', 'Short.Name')], by = 'SCC')
# Sum the Coal emissions for each year
emissions_year_coal <- aggregate(Emissions ~ year, coal, sum, na.rm = T)
png('pics/plot4.png')# , width = 480, height = 480, units = 'px'
# Plot the chart
plot(emissions_year_coal, type = 'o', main = 'Total Coal combustion-related sources emission in United States', xlab = 'Year of emissions recorded', ylab = 'Amount of Coal combustion-related sources emitted, in tons')
dev.off()
