...
```{r sfo-feb-flights}
# type your code for Question 1 here, and Knit : 68
...
# type your code for Question 2 here, and Knit : No flight is delayed more than 2 hours.
...
# type your code for Question 3 here, and Knit : Delta and United Airlines
sfo_feb_flights %>%
    group_by(carrier) %>%
    summarise(median_ad = median(arr_delay), iqr_qr = IQR(arr_delay)) %>%
    arrange(desc(iqr_qr))
```
...
# type your code for Question 4 here, and Knit : July
nycflights %>%
  group_by(month) %>%
  summarise(mean_dd = mean(dep_delay)) %>%
  arrange(desc(mean_dd)) %>%
...
# type your code for Question 5 here, and Knit : December
nycflights %>%
  group_by(month) %>%
...
```{r q6}
# type your code for Question 6 here, and Knit : Median would be more reliable as the distribution of delays is skewed.
```
...
# type your code for Question 7 here, and Knit : LGA
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = sum(dep_type == "on time") / n()) %>%
  arrange(desc(ot_dep_rate)) %>%
...
# type your code for Question 8 here, and Knit : N666DN
nycflights %>%
  group_by(flight) %>%
...
# type your code for Question 9 here, and Knit : There is an overall postive association between distance and average speed.
avg_speed_nycflights <- nycflights %>%
  group_by(flight) %>%
  mutate(avg_speed = (distance / (air_time / 60))) %>%
  arrange(distance)
ggplot(avg_speed_nycflights, aes(x = distance, y = avg_speed)) +
  geom_point()
```
