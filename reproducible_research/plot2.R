...
get_plot <- function(i)
    with(sp_data[[i]], plot(log(Average.Covered.Charges), log(Average.Total.Payments), main = names(df_data[i]), col = DRG.Definition))
...
with(data, legend("bottom", legend = levels(DRG.Definition), pch = 1, col = seq_along(levels(DRG.Definition)), xpd = TRUE))
